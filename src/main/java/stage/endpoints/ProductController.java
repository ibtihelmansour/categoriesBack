...
public class ProductController {
	...
	@CrossOrigin("http://localhost:4200")
	@PutMapping("/{id}")
	public Product modifProduct ( @PathVariable("id") long id , @RequestBody Product product) { 
		  Product product2 = this.findById(id) ; 
		   product2.setDateofmodify(new Timestamp(System.currentTimeMillis()));
			if (product.getProductNom() != null) 
				product2.setProductNom(product.getProductNom() );
			if( product.getQt() > 0 ) { 
				product2.setQt(product.getQt()) ; 
